...
const GEAR_ASSETS = [
  { id: 'clapper-1', type: 'clapper', x: 15, y: 20, size: 120, rotation: -15 },
  { id: 'clapper-2', type: 'clapper', x: 80, y: 70, size: 100, rotation: 10 },
  { id: 'reel-1',    type: 'reel',    x: 10, y: 80, size: 280, rotation: 0 },
  { id: 'reel-2',    type: 'reel',    x: 85, y: 15, size: 320, rotation: 0 },
  { id: 'camera-1',  type: 'camera',  x: 50, y: 40, size: 400, rotation: 0 },
];

// Seeded pseudo-random — deterministic, zero hydration mismatch
function sr(seed: number): number {
  const x = Math.sin(seed + 1) * 10000;
  return x - Math.floor(x);
}

interface Particle { id: number; left: number; top: number; scale: number; }

export default function MovieSetBackground() {
  const containerRef = useRef<HTMLDivElement>(null);

  // Particles generated ONLY after mount (client-safe, never SSR)
  ...
  useGSAP(() => {
    // Clapperboard snap
    gsap.to('.clapper-top', {
      rotate: -30, duration: 0.6, repeat: -1, yoyo: true, ease: 'power2.inOut',
      stagger: { each: 1, repeatRefresh: true }
    });

    // Reel rotation
    gsap.to('.film-reel', { rotate: 360, duration: 10, repeat: -1, ease: 'none' });

    // Light beam sweep
    gsap.to('.light-beam', {
      rotate: '+=20', duration: 8, repeat: -1, yoyo: true, ease: 'sine.inOut', stagger: 2
    });

    // Particle drift
    gsap.to('.set-particle', {
      y: '+=100', x: '+=50', opacity: 0, duration: 8, repeat: -1, ease: 'none',
      stagger: { amount: 5, from: 'start' }
    });

    // Scroll parallax
    gsap.to('.set-element', {
      y: (_i, el) => -200 * parseFloat(el.getAttribute('data-depth') || '1'),
    ...
    <div ref={containerRef} className="fixed inset-0 pointer-events-none z-[-2] overflow-hidden bg-transparent">
      ...
      <div className="absolute inset-0 z-0">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="light-beam absolute top-[-20%] w-[400px] h-[150%] opacity-[0.12]"
            style={{
              left: `${i * 30 - 15}%`,
      ...
      {/* Dust Particles — client-only, zero hydration mismatch */}
      <div className="absolute inset-0">
        {particles?.map((p) => (
          <div
            key={p.id}
            className="set-particle absolute w-2 h-2 bg-[#6a4a8c]/10 rounded-full blur-[1px] will-change-transform"
